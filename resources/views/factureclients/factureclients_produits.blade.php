...
<div class="row">
    <div class="col-md-6 m-3 mx-auto shadow p-2">
        <form action="{{ route('factureclients.produits.store', $factureclients->id) }}" method="POST">
            @csrf
            <div class="mb-3">
...
<div class="table_section padding_infor_info">
    <div class="table-responsive-sm">
        <h4>Commande Num : {{ $factureclients->id }}</h4>
        <table class="table table-bordered" id="commandes">
            ...
            <tbody>
                @foreach ($factureclients->produit as $f)
                  <tr>
                    <th scope="row">{{ $f->id }}{{ $factureclients->id }}</th>
                    <td>{{ $f->pivot->qte }}</td>
                    <td>{{ $f->pivot->prix }}</td>
                    <td>
                        {{$f->libelle}}
                    ...
                    <td class="btn-group">
                         <form method="post" action="{{ route('factureclients.produits.destroy', $f->id) }}"
                        ...
                        <a class="btn btn-warning" href="{{ route('factureclients.produits', $f) }}"><i
